...
> **当前进度（2026-07-18 第7次）**: M1–M25 全部完成 + **M26 内容版本回滚**（PRD §3.3 P0 版本管理·历史版本与回滚 / US-02 验收标准 "支持历史版本查看和回滚"）：后端 `ContentService.rollbackVersion()` — 查找目标版本（`contentVersion.findFirst`，未找到 `NotFoundException`），事务内把目标版本的 title/body/contentType 恢复到 live content、版本计数器 +1，并把「回滚」本身记为一条新的 `ContentVersion`（`changeNote` 默认 `Rolled back to v{n}`），实现回滚可逆 + 可审计；新增 `RollbackVersionDto`（`@IsInt @Min(1) version` + 可选 `@MaxLength(500) changeNote`）；`POST /contents/:id/rollback`（`JwtAuthGuard` + `AuditService` 记录 `ROLLBACK` 操作）；`test/prisma.mock.ts` 补 `contentVersion.findFirst`；后端单测 **+7**（服务 4：字段恢复 + 版本快照、自定义 changeNote、目标版本不存在 NotFound、$transaction 写入目标字段 / 控制器 3：委托 + audit、changeNote 透传）；前端 `contents/[id]` 版本历史卡片每行新增「Roll back」按钮（`window.confirm` 二次确认、`editing` 中禁用、忙态、`POST /contents/${id}/rollback` 后 reload）。测试 **328 通过 / 31 API 套件**（+7），API + web typecheck 与 build 全绿。
