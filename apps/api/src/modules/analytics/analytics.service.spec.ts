...
import { AnalyticsReportService } from './report/report.service';
...
describe('AnalyticsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module = await Test.createTestingModule({
      providers: [
        ...
        { provide: AnalyticsReportService, useValue: {
          getAvailableFields: jest.fn().mockReturnValue({ categories: [] }),
  ...
  // Report logic delegated to AnalyticsReportService (extracted for maintainability)

  describe('getAvailableFields', () => {
    it('delegates to AnalyticsReportService', () => {
      const mockResult = { categories: [{ category: 'account', fields: [{ id: 'f1', label: 'F1', category: 'account', type: 'number' }] }] };
      (service as any).reports.getAvailableFields.mockReturnValue(mockResult);
      const result = service.getAvailableFields();
      expect((service as any).reports.getAvailableFields).toHaveBeenCalledTimes(1);
      expect(result).toBe(mockResult);
    });
  });

  describe('generateReport', () => {
    it('delegates to AnalyticsReportService with correct args', async () => {
      const mockResult = { fields: [], rows: [], totalCount: 0, generatedAt: '' };
      (service as any).reports.generateReport.mockResolvedValue(mockResult);

      const fieldIds = ['platform', 'impressions'];
      const filters = [{ field: 'platform', operator: 'eq', value: 'WECHAT_OFFICIAL' }] as any;
      const result = await service.generateReport(fieldIds, filters, 'platform', 'impressions', 'desc', 50);

      expect((service as any).reports.generateReport).toHaveBeenCalledWith(fieldIds, filters, 'platform', 'impressions', 'desc', 50);
      expect(result).toBe(mockResult);
    });
  });

  describe('saveReport / listReports / getReport / deleteReport', () => {
    it('saveReport delegates to AnalyticsReportService', async () => {
      (service as any).reports.saveReport.mockResolvedValue({ id: 'r1' });
      const result = await service.saveReport('t1', 'u1', { name: 'My report', fieldIds: ['platform'] });
      expect((service as any).reports.saveReport).toHaveBeenCalledWith('t1', 'u1', { name: 'My report', fieldIds: ['platform'] });
      expect(result).toHaveProperty('id', 'r1');
    });

    it('listReports delegates to AnalyticsReportService', async () => {
      (service as any).reports.listReports.mockResolvedValue([{ id: 'r1' }]);
      const result = await service.listReports('t1');
      expect((service as any).reports.listReports).toHaveBeenCalledWith('t1');
      expect(result).toEqual([{ id: 'r1' }]);
    });

    it('getReport delegates to AnalyticsReportService', async () => {
      (service as any).reports.getReport.mockResolvedValue({ id: 'r1' });
      const result = await service.getReport('r1');
      expect((service as any).reports.getReport).toHaveBeenCalledWith('r1');
      expect(result).toHaveProperty('id', 'r1');
    });

    it('deleteReport delegates to AnalyticsReportService', async () => {
      (service as any).reports.deleteReport.mockResolvedValue({ success: true, deletedId: 'r1' });
      const result = await service.deleteReport('r1');
      expect((service as any).reports.deleteReport).toHaveBeenCalledWith('r1');
