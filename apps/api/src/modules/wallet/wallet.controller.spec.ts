...
describe('WalletController', () => {
  ...
  it('delegates balance lookup to the service', async () => {
    ...
    const res = await controller.balance('team-1', mockUser as any);
    expect(service.balance).toHaveBeenCalledWith('team-1', 'u1');
  ...
  it('delegates top-up with team from path', async () => {
    ...
    const res: any = await controller.topUp('team-1', mockUser as any, { amount: 200, note: 'seed' } as any);
    expect(service.topUp).toHaveBeenCalledWith('team-1', { amount: 200, note: 'seed' }, 'u1');
  ...
  it('parses debit type and delegates', async () => {
    ...
    const res: any = await controller.debit('team-1', mockUser as any, { type: 'PUBLISH', refId: 'j1' } as any);
    expect(service.debit).toHaveBeenCalledWith('team-1', 'PUBLISH', 'u1', { refId: 'j1', note: undefined });
  ...
  it('returns a zero-charge response for free operations', async () => {
    ...
    const res: any = await controller.debit('team-1', mockUser as any, { type: 'TOPUP' } as any);
    expect(res.charge).toBe(0);
  });

  it('throws NotFoundException for an unknown transaction type', async () => {
    await expect(
      controller.debit('team-1', mockUser as any, { type: 'NOT_A_TYPE' } as any),
    ).rejects.toBeInstanceOf(NotFoundException);
  });

  it('passes pagination to listTransactions', async () => {
    await controller.listTransactions('team-1', mockUser as any, { skip: 5, take: 10 } as any);
    expect(service.listTransactions).toHaveBeenCalledWith('team-1', 'u1', { skip: 5, take: 10 });
  });
